...
void array_resize(t_array *x, int new_size)
{
    t_template *template = template_findbyname(x->a_templatesym);
    if(new_size < 1) new_size = 1;
    int old_size = x->a_n;
    int elemsize = sizeof(t_word) * template->t_n;

    char *tmp = (char *) resizebytes(
        x->a_vec, old_size * elemsize, new_size * elemsize);
    if(!tmp) return;
    x->a_vec = tmp;
    x->a_n = new_size;
    if(new_size > old_size)
    {
        char *cp = x->a_vec + elemsize * old_size;
        for(int i = 0; i < new_size - old_size; i++, cp += elemsize)
        {
            t_word *wp = (t_word *) cp;
...
void array_resize_and_redraw(t_array *array, t_glist *glist, int new_size)
{
    t_array *a2 = array;
    int is_visible = glist_isvisible(glist);
    while(a2->a_gp.gp_stub->gs_which == GP_ARRAY)
        a2 = a2->a_gp.gp_stub->gs_un.gs_array;
    if(is_visible) gobj_vis(&a2->a_gp.gp_un.gp_scalar->sc_gobj, glist, 0);
    ...
    t_floatarg fsize, t_floatarg fflags)
{
    ...
    t_symbol *templatesym = gensym("pd-float-array");
    t_template *template = template_findbyname(templatesym);
    if(!template)
    {
        pd_error(0, "array: couldn't find template %s", templatesym->s_name);
    ...
    int saveit = ((flags & GRAPH_ARRAY_SAVE) != 0);
    int savesize = ((flags & GRAPH_ARRAY_SAVESIZE) != 0);
    t_garray *x = graph_scalar(gl, s, templatesym, saveit, savesize);
