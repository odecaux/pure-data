    ...
    for(int i = 0; i < x->x_n; i++)
    {
        ...
        else if(ch >= 0 && ch < sys_get_outchannels())
        {
            dsp_add(plus_perform, 4, STUFF->st_soundout + DEFDACBLKSIZE * ch,
                sp[i]->s_vec, STUFF->st_soundout + DEFDACBLKSIZE * ch,
                (t_int) DEFDACBLKSIZE);
        }
    }
}
...
static void adc_dsp(t_adc *x, t_signal **sp)
{
    for(int i = 0; i < x->x_n; i++)
    {
        ...
        {
            dsp_add_copy(STUFF->st_soundin + DEFDACBLKSIZE * ch, sp[i]->s_vec,
                DEFDACBLKSIZE);
        }
        else
        {
            dsp_add_zero(sp[i]->s_vec, DEFDACBLKSIZE);
        }
    }
}
