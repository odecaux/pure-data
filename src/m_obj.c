...
t_inlet *floatinlet_new(t_object *owner, t_float *fp)
{
    ...
    t_inlet *owner_inlet = owner->ob_inlet;
    if(owner_inlet)
    {
        t_inlet *temp;
        while((temp = owner_inlet->i_next))
            owner_inlet = temp;
        owner_inlet->i_next = x;
    }
    else
    {
        owner->ob_inlet = x;
...
t_outlet *outlet_new(t_object *owner, t_symbol *s)
{
    ...
    t_outlet *owner_outlet = owner->ob_outlet;
    if(owner_outlet)
    {
        t_outlet *temp;
        while((temp = owner_outlet->o_next))
            owner_outlet = temp;
        owner_outlet->o_next = x;
    }
    else
    {
        owner->ob_outlet = x;
...
void outlet_free(t_outlet *x)
{
    ...
    // beginning of the outlet list
    if(owner->ob_outlet == x)
    {
        owner->ob_outlet = x->o_next;
    }
    else
    {
        for(t_outlet *x2 = owner->ob_outlet; x2; x2 = x2->o_next)
        {
            // stitch together nodes before and after
            if(x2->o_next == x)
            {
                x2->o_next = x->o_next;
                break;
...
t_outconnect *obj_connect(t_object *source, int outno, t_object *sink, int inno)
{
    ...
    for(o = source->ob_outlet; o && outno; o = o->o_next, outno--)
        ;
    // out of bounds outno or no outlet ?
    if(!o) return (0);
    ...
    for(i = sink->ob_inlet; i && inno; i = i->i_next, inno--)
        ;
    // out of bounds inno or no inlet ?
    if(!i) return (0);
    to = &i->i_pd;

doit:
    t_outconnect **ochead = outlet_getconnectionpointer(o);
    t_outconnect *oc = (t_outconnect *) t_getbytes(sizeof(*oc));
    oc->oc_next = 0;
    oc->oc_to = to;
    /* append it to the end of the list */
    /* LATER we mcight cache the last "oc" to make this faster. */
    t_outconnect *last_oc = *ochead;
    if(last_oc)
    {
        while(last_oc->oc_next)
            last_oc = last_oc->oc_next;
        last_oc->oc_next = oc;
    }
    else
    {
...
doit:
    ...
    t_outconnect *next_oc;
    while((next_oc = oc->oc_next))
    {
        // if found, stitch oc before and after
        if(next_oc->oc_to == to)
        {
            oc->oc_next = next_oc->oc_next;
            freebytes(next_oc, sizeof(*next_oc));
            goto done;
        }
        oc = next_oc;
    }
