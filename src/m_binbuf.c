...
void binbuf_text(t_binbuf *x, const char *text, size_t size)
{
    ...
    binbuf_clear(x);
    if(!binbuf_resize(x, num_alloc)) return;
    ...
    while(1)
    {
        int type;
        /* skip leading space */
        while((textp != text_end) && (*textp == ' ' || *textp == '\n' ||
                                         *textp == '\r' || *textp == '\t'))
            textp++;
        if(textp == text_end) break;
        if(*textp == ';')
        {
            SETSEMI(ap);
            textp++;
        ...
        else
        {
            ...
            do
            {
                ...
                *bufp = *textp++;
                ...
                if(!lastslash && c == '$' &&
                    (textp != text_end && textp[0] >= '0' && textp[0] <= '9'))
                    dollar = 1;
                if(!slash)
                {
                    bufp++;
            ...
            } while(textp != text_end && bufp != buf_end &&
                    (slash ||
                        (*textp != ' ' && *textp != '\n' && *textp != '\r' &&
                            *textp != '\t' && *textp != ',' && *textp != ';')));
            *bufp = 0;
        ...
        ap++;
        num_atom++;
        if(num_atom == num_alloc)
        {
            if(!binbuf_resize(x, num_alloc * 2)) break;
            num_alloc = num_alloc * 2;
            ap = x->b_vec + num_atom;
        }
        if(textp == text_end) break;
    }
    /* reallocate the vector to exactly the right size */
    binbuf_resize(x, num_atom);
}
