...
static t_int *vline_tilde_perform(t_int *w)
{
    ...
    double initial_value = x->x_value;
    double inc = x->x_inc;
    double msecpersamp = x->x_msecpersamp;
    double logicaltimenow = clock_gettimesince(x->x_referencetime);
    t_vseg *segment = x->x_list;

    //????
    if(logicaltimenow != x->x_lastlogicaltime)
    {
        int sampstotime =
            (num_samples > DEFDACBLKSIZE ? num_samples : DEFDACBLKSIZE);
        x->x_lastlogicaltime = logicaltimenow;
        x->x_nextblocktime = logicaltimenow - sampstotime * msecpersamp;
    }

    double timenow = x->x_nextblocktime;
    x->x_nextblocktime = timenow + num_samples * msecpersamp;

    t_sample current_value = initial_value;
    for(int i = 0; i < num_samples; i++)
    {
        double time_next_sample = timenow + msecpersamp;

        while(segment && segment->s_starttime < time_next_sample)
        {
            if(x->x_targettime <= time_next_sample)
            {
                current_value = x->x_target;
                inc = 0;
            }
            /* if zero-length segment bash output value */
            // weird hmmmm
            if(segment->s_targettime <= segment->s_starttime)
            {
                current_value = segment->s_target;
                inc = 0;
            }
            else
            {
                // slope : (y2 - y1) / (x2 - x1)
                double incpermsec =
                    (segment->s_target - current_value) /
                    (segment->s_targettime - segment->s_starttime);
                current_value =
                    current_value +
                    incpermsec * (time_next_sample - segment->s_starttime);
                inc = incpermsec * msecpersamp;
            }

            x->x_inc = inc;
            x->x_target = segment->s_target;
            x->x_targettime = segment->s_targettime;

            x->x_list = segment->s_next;
            // TODO list allocator
            t_freebytes(segment, sizeof(*segment));
            segment = x->x_list;
        }
        ...
        out[i] = current_value;
        current_value += inc;
        timenow = time_next_sample;
    }
    x->x_value = current_value;
    return (w + 4);
}
...
static void vline_tilde_stop(t_vline *x)
{
    t_vseg *seg;
    t_vseg *next;
    for(seg = x->x_list; seg; seg = next)
    {
        next = seg->s_next;
        t_freebytes(seg, sizeof(*seg));
    }
    x->x_list = 0;
    x->x_inc = 0;
...
static void vline_tilde_float(t_vline *x, t_float f)
{
    ...
    // different list deletion style from previous function
    while(deletefrom)
    {
...
static t_int *vsnapshot_tilde_perform(t_int *w)
{
    ...
    int num_samples_to_snap = x->x_n;
    for(int i = 0; i < num_samples_to_snap; i++)
    {
        out[i] = in[i];
    }
...
{
    int input_num_samples = sp[0]->s_n;
    int snapshot_num_samples = x->x_n;
    if(input_num_samples != snapshot_num_samples)
    {
        if(x->x_vec) t_freebytes(x->x_vec, x->x_n * sizeof(t_sample));
        x->x_vec = (t_sample *) getbytes(input_num_samples * sizeof(t_sample));
        x->x_gotone = 0;
        x->x_n = input_num_samples;
    }
...
{
    int window_size = fnpoints;
    int period = fperiod;

    if(window_size < 1) window_size = 1024;
    if(period < 1) period = window_size / 2;
    if(period < window_size / MAXOVERLAP + 1)
        period = window_size / MAXOVERLAP + 1;

    t_sample *hanning;
    if(!(hanning = getbytes(sizeof(t_sample) * (window_size + INITVSTAKEN))))
    {
    ...
    for(int i = 0; i < window_size; i++)
        hanning[i] = (1. - cos((2 * 3.14159 * i) / window_size)) / window_size;
    for(int i = window_size; i < window_size + INITVSTAKEN; i++)
        hanning[i] = 0;
...
static t_int *env_tilde_perform(t_int *w)
{
    ...
    int num_samples = (int) (w[3]);
    t_sample *sum = x->x_sumbuf;
    t_sample *hanning = x->x_buf;

    int sum_idx = 0;
    for(int window_phase = x->x_phase; window_phase < x->x_npoints;
        window_phase += x->x_realperiod, sum_idx++)
    {

        for(int i = 0; i < num_samples; i++)
        {
            sum[sum_idx] += hanning[window_phase + i] * (in[i] * in[i]);
        }
    }
    sum[sum_idx] = 0;

    x->x_phase += num_samples;
    if(x->x_phase >= x->x_realperiod)
    {
        /* output */
        x->x_result = x->x_sumbuf[0];

        // shift sum buffer left
        sum = x->x_sumbuf;
        sum_idx = 0;
        for(int window_phase = x->x_realperiod; window_phase < x->x_npoints;
            window_phase += x->x_realperiod, sum_idx++)
        {
            sum[sum_idx] = sum[sum_idx + 1];
        }
        sum[sum_idx] = 0;
        x->x_phase -= x->x_realperiod;
