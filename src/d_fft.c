...
static void sigfft_dspx(t_sigfft *x, t_signal **sp, t_int *(*f)(t_int *w))
{
    ...
    {
        dsp_add(sigfft_swap, 3, out1, out2, (t_int) num_samples);
    }
    else if(out1 == in2)
    {
        dsp_add(copy_perform, 3, in2, out2, (t_int) num_samples);
        dsp_add(copy_perform, 3, in1, out1, (t_int) num_samples);
    }
    else
    {
        if(out1 != in1)
    ...
    dsp_add(f, 3, sp[2]->s_vec, sp[3]->s_vec, (t_int) num_samples);
}
...
static t_int *sigframp_perform(t_int *w)
{
    ...
    t_sample fbin = 1;
    t_sample oneovern2 =
        1.f / ((t_sample) num_samples * (t_sample) num_samples);
    ...
    *outamp++ = *outfreq++ = 0;
    ...
    {
        int lastreal = currentreal;
        currentreal = nextreal;
        nextreal = *inreal++;
        int lastimag = currentimag;
        currentimag = nextimag;
        nextimag = *inimag++;
        t_sample re = currentreal - 0.5f * (lastreal + nextreal);
        t_sample im = currentimag - 0.5f * (lastimag + nextimag);
        t_sample pow = re * re + im * im;
        t_sample freq;
        if(pow > 1e-19)
        {
            t_sample detune =
