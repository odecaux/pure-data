...
static void *tabwrite_tilde_new(t_symbol *name)
{
...
static void tabwrite_tilde_set(t_tabwrite_tilde *x, t_symbol *name)
{
    t_garray *a = (t_garray *) pd_findbyclass(x->x_arrayname, garray_class);
    ...
    {
        if(*name->s_name)
            pd_error(x, "tabwrite~: %s: no such array", name->s_name);
        x->x_vec = 0;
    }
    else if(!garray_getfloatwords(a, &x->x_nsampsintab, &x->x_vec))
    {
        pd_error(x, "%s: bad template for tabwrite~", name->s_name);
        x->x_vec = 0;
    }
    else
        garray_usedindsp(a);
...
static void *tabplay_tilde_new(t_symbol *name)
{
...
static void tabplay_tilde_set(t_tabplay_tilde *x, t_symbol *name)
{
    t_garray *a = (t_garray *) pd_findbyclass(x->x_arrayname, garray_class);
    ...
    {
        if(*name->s_name)
            pd_error(x, "tabplay~: %s: no such array", name->s_name);
        x->x_vec = 0;
    }
    else if(!garray_getfloatwords(a, &x->x_nsampsintab, &x->x_vec))
    {
        pd_error(x, "%s: bad template for tabplay~", name->s_name);
        x->x_vec = 0;
    }
    else
        garray_usedindsp(a);
...
static void *tabread_tilde_new(t_symbol *name)
{
...
static void tabread_tilde_set(t_tabread_tilde *x, t_symbol *name)
{
    t_garray *a;
    ...
    {
        if(*name->s_name)
            pd_error(x, "tabread~: %s: no such array", name->s_name);
        x->x_vec = 0;
    }
    else if(!garray_getfloatwords(a, &x->x_npoints, &x->x_vec))
    {
        pd_error(x, "%s: bad template for tabread~", name->s_name);
        x->x_vec = 0;
    }
    else
        garray_usedindsp(a);
...
static void *tabread4_tilde_new(t_symbol *name)
{
...
static void tabread4_tilde_set(t_tabread4_tilde *x, t_symbol *name)
{
    t_garray *a;
    ...
    {
        if(*name->s_name)
            pd_error(x, "tabread4~: %s: no such array", name->s_name);
        x->x_vec = 0;
    }
    else if(!garray_getfloatwords(a, &x->x_npoints, &x->x_vec))
    {
        pd_error(x, "%s: bad template for tabread4~", name->s_name);
        x->x_vec = 0;
    }
    else
        garray_usedindsp(a);
...
static void *tabosc4_tilde_new(t_symbol *name)
{
...
static void tabosc4_tilde_set(t_tabosc4_tilde *x, t_symbol *name)
{
    t_garray *a;
    int npoints;
    int pointsinarray;
...
static void *tabsend_new(t_symbol *name)
{
...
static void tabsend_set(t_tabsend *x, t_symbol *name)
{
    t_garray *a;
    ...
    {
        if(*name->s_name)
            pd_error(x, "tabsend~: %s: no such array", name->s_name);
        x->x_vec = 0;
    }
    else if(!garray_getfloatwords(a, &x->x_npoints, &x->x_vec))
    {
        pd_error(x, "%s: bad template for tabsend~", name->s_name);
        x->x_vec = 0;
    }
    else
        garray_usedindsp(a);
...
static void tabreceive_set(t_tabreceive *x, t_symbol *name)
{
    t_garray *a;
    ...
    {
        if(*name->s_name)
            pd_error(x, "tabreceive~: %s: no such array", name->s_name);
        x->x_vec = 0;
    }
    else if(!garray_getfloatwords(a, &x->x_npoints, &x->x_vec))
    {
        pd_error(x, "%s: bad template for tabreceive~", name->s_name);
        x->x_vec = 0;
    }
    else
        garray_usedindsp(a);
...
static void *tabreceive_new(t_symbol *name)
{
...
static void tabread_set(t_tabread *x, t_symbol *name) { x->x_arrayname = name; }

static void *tabread_new(t_symbol *name)
{
...
static void tabread4_set(t_tabread4 *x, t_symbol *name)
{
    x->x_arrayname = name;
}

static void *tabread4_new(t_symbol *name)
{
...
static void tabwrite_set(t_tabwrite *x, t_symbol *name)
{
    x->x_arrayname = name;
}

static void *tabwrite_new(t_symbol *name)
{
